...
DEFAULT_LENGTH_TYPE_COLUMN_CANDIDATES = [
    "Input length type",
    "Length Category",
    "Category",
    "Length",
...
def _get_length_category(text: str) -> str:
    length = len(text)
    if length <= 30:
        return "S"
    elif 31 <= length <= 299:
...
def _find_column_index(header_values: list, requested_name: str | None, candidates: list[str]) -> int | None:
    ...
    def find_partial(name: str) -> int | None:
        n = _normalize_header(name)
        if not n:
            return None
        for i, v in indexed:
    ...
    if requested_name:
        found = find_exact(requested_name)
        if found:
            return found
    ...
    if requested_name:
        found = find_partial(requested_name)
        if found:
            return found

    for c in candidates:
        found = find_partial(c)
        if found:
            return found
...
def _parse_args():
    ...
    parser.add_argument("--length-type-col", default=None)
...
def run_test():
    ...
    length_type_col_idx = _find_column_index(header_values, args.length_type_col, DEFAULT_LENGTH_TYPE_COLUMN_CANDIDATES)
    length_type_col_idx = length_type_col_idx or _ensure_column(ws, header_row, header_values, "Input length type")
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            length_cat = _get_length_category(singlish_input)
            _set_cell_value(ws, row_index, length_type_col_idx, length_cat)
